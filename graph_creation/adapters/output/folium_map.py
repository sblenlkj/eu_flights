...
class FoliumMapExporter(GraphExporter):
    def export(self, graph: Graph, file_name: str) -> None:
        output_path = build_output_path(file_name, default_ext='.html', postfix='_map', out_dir='data')
        ...
        for node in graph.nodes:
            folium.CircleMarker(
                ...
                radius=2 + node_weight_to_size_translator(node.weight),
                popup=f"{node.name} ({node.weight})",
                fill=True,
            ).add_to(fmap)
